...
try:
    import argparse
except ImportError: # Fails if Python < 2.7
    raise RuntimeError("Script should be used with Python 2.7 or higher.  Sorry...")
...
def main(srcdirectory):
    ...
    path_dict = { os.path.join(srcdirectory,"gtk-2.0","gtk")  : "gtk-auto.f90",
                  os.path.join(srcdirectory,"gtk-2.0","gdk")  : "gdk-auto.f90",
                  os.path.join(srcdirectory,"cairo")          : "cairo-auto.f90",
                  os.path.join(srcdirectory,"pango-1.0")      : "pango-auto.f90",
                  os.path.join(srcdirectory,"glib-2.0")       : "glib-auto.f90",
                  os.path.join(srcdirectory,"gdk-pixbuf-2.0") : "gdk-pixbuf-auto.f90",
                  os.path.join(srcdirectory,"atk-1.0")        : "atk-auto.f90",}
...
if __name__ == "__main__":
    aparse = argparse.ArgumentParser(description="Generates Fortran interfaces (*-auto.f90) from Gtk+ C headers")
    aparse.add_argument('directory', metavar='directory', type=str,
                        help='base directory where Gtk+ include directories live',
                        default=os.path.join('usr','include'))
